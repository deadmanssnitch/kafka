...
RSpec.describe Kafka::FFI do
  ...
  specify ".rd_kafka_msg_partitioner_fnv1a" do
    client = Kafka::FFI::Producer.new(config)

    with_topic(partitions: 5) do |topic|
      t = client.topic(topic)

      part = Kafka::FFI.rd_kafka_msg_partitioner_fnv1a(t, "key", 3, 5, nil, nil)
      expect(part).to eq(3)

      # nil / empty key is always assigned partition 0
      part = Kafka::FFI.rd_kafka_msg_partitioner_fnv1a(t, nil, 0, 5, nil, nil)
      expect(part).to eq(0)
    end
  ensure
    client.destroy
  ...
  specify ".rd_kafka_msg_partitioner_fnv1a_random" do
    client = Kafka::FFI::Producer.new(config)

    with_topic(partitions: 5) do |topic|
      t = client.topic(topic)

      part = Kafka::FFI.rd_kafka_msg_partitioner_fnv1a_random(t, "key", 1, 5, nil, nil)
      expect(part).to eq(1)

      # nil / empty key is assigned a random partition
      part = Kafka::FFI.rd_kafka_msg_partitioner_fnv1a_random(t, nil, 0, 5, nil, nil)
      expect(part).to be < 5
      expect(part).to be >= 0
    end
  ensure
